...
export default function WebsiteEditor() {
    ...
    useEffect(() => {
        ...
        const i = setInterval(() => {
            setThinkingIndex((i) => (i + 1) % thinkinSteps.length)
        }, 1200)
    ...
    if (error) {
        return <div className='h-screen flex items-center justify-center bg-black text-red-400'>
            {error}
        </div>
    }
        ...
        <div className='h-screen w-screen flex bg-black text-white overflow-hidden'>
            <aside className='hidden lg:flex w-[380px] flex-col border-r border-white/10 bg-black/80 '>
                ...
                <>
                    <div className='flex-1 overflow-y-auto px-4 space-y-4'>
                        ...
                        border-white/10 text-zinc-400 italic'>{thinkinSteps[thinkingIndex]}</div>
                            </div>}
                    ...
                    <div className='p-3 border-t border-white/10'>
                        ...
                        <div className='flex gap-2'>
                            <input placeholder='Describe Changes...' className='flex-1 resize-none rounded-2xl px-4
                    py-3 bg-white/5 border border-white/10 text-outline-none' onChange={(e) => setPrompt(e.target.value)} value={prompt} />
            ...
            <div className='flex-1 flex flex-col'>
                <div className='h-14 px-4 flex justify-between items-center border-b border-white/10 bg-black/80'>
                    <span className='text-xs text-zinc-400'>Live Privew</span>
                    <div className='flex gap-2'>
                        {website?.deployed ? (""):( <> <button className='flex items-center gap-2 px-4 py-1.5 rounded-lg bg-linear-to-r from-indigo-500
            ...
            <AnimatePresence>
                {showFullPreview && (
                    <motion.div className='fixed inset-0 z-[9999] bg-black '>
                        <iframe className='w-full h-full bg-white' srcDoc={code} sandbox='allow-scripts allow-origin allow-forms' />
